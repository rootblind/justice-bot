...
import { attach_flagged_message_collector } from "../../Systems/automoderation/automodmoderation_system.js";
...
const messageCreate: Event = {
    ...
    async execute(message: Message) {
        ...
        const isChannelIgnore = await ServerLogsIgnoreRepo.isChannelIgnored(guild.id, channel.id);
        if(isChannelIgnore) return;
        ...
        if(isModelOnline && flaggedMessagesLogs) {
            ...
            if(response) { // if the message was detected to be toxic
                ...
                if(message.content.length <= 3000) {
                    flaggedEmbed.setDescription(`**Content**: ${message.content}`);
                } else {
