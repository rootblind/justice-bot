...
const clientReady: Event = {
    ...
    async execute(client: Client) {
        ...
        try {
            await ensureDirectories(local_config.sources.system_directories);
        } catch (error) {
            await errorLogHandle(error, "", "Fatal error");
            setTimeout(() => process.exit(1), 5_000);
        }
        ...
        */
        ...
        if (local_config.sources.self_flag_data) {
            // setting a csv file for the data collection of the moderation model
            // this dataset is used for self labeling
            const flagDataFileExists = await isFileOk(local_config.sources.self_flag_data);
            if (!flagDataFileExists) {
                await fs.promises.writeFile(local_config.sources.self_flag_data,
                    'Message,OK,Aggro,Violence,Sexual,Hateful\n',
                    "utf-8"
                );
            }
