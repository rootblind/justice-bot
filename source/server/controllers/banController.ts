import { fetchBan, fetchGuild, fetchMember } from "../../utility_modules/discord_helpers";
...
import { error_logger } from "../../utility_modules/error_logger";
...
export const getBan = async (req: Request, res: Response, client: Client) => {
    ...
    const ban = await fetchBan(guild, String(ban_id));
    ...
    try{
        const banData = await BanListRepo.getUserBan(guild.id, ban.user.id);

        if(banData) {
            const moderator = await fetchMember(guild, String(banData.moderator));
