module.exports = (existing, local) => {
    ...
    function optionsArray(cmd) {
        ...
        return (cmd.options || []).map((option) => {
            ...
            cleanedOption.options ? (cleanedOption.options = normalizeObject(cleanedOption.options)) :
                                    (cleanedOption.options = normalizeObject(cleanedOption));
            cleanObject(cleanedOption);
            return {
                ...cleanedOption,
                choices: cleanedOption.choices
                ? JSON.stringify(cleanedOption.choices.map((c) => c.value)) : null
            }
